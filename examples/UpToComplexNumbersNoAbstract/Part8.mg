...
Theorem pos_small_real_recip_ex: forall x :e real, 0 < x -> x < 1 -> exists y :e real, x * y = 1.
let x. assume Hx Hxpos Hxsmall.
apply dneg.
assume HC: ~exists y :e real, x * y = 1.
apply real_E x Hx.
assume Hx1 Hx2 Hx3 Hx4 Hx5 Hx6 Hx7.
set L := {q :e SNoS_ omega|x * q < 1}.
set R := {q :e SNoS_ omega|1 < x * q}.
claim LLE: forall q :e L, forall p:prop, (SNo q -> SNoLev q :e omega -> x * q < 1 -> p) -> p.
{ let q. assume Hq: q :e L. let p. assume Hp.
  apply SepE (SNoS_ omega) (fun q => x * q < 1) q Hq.
  assume Hq1 Hq2.
  apply SNoS_E2 omega omega_ordinal q Hq1.
  assume Hq1a _ Hq1c _.
  exact Hp Hq1c Hq1a Hq2.
}
claim LRE: forall q :e R, forall p:prop, (SNo q -> SNoLev q :e omega -> 1 < x * q -> p) -> p.
{ let q. assume Hq: q :e R. let p. assume Hp.
  apply SepE (SNoS_ omega) (fun q => 1 < x * q) q Hq.
  assume Hq1 Hq2.
  apply SNoS_E2 omega omega_ordinal q Hq1.
  assume Hq1a _ Hq1c _.
  exact Hp Hq1c Hq1a Hq2.
...
{ prove (forall w :e L, SNo w)
     ...
     /\ (forall w :e L, forall z :e R, w < z).
  apply and3I.
  - let w. assume Hw. apply LLE w Hw. assume H _ _. exact H.
  - let z. assume Hz. apply LRE z Hz. assume H _ _. exact H.
  - let w. assume Hw. let z. assume Hz.
    ...
    assume Hz3: 1 < x * z.
    prove w < z.
    apply SNoLtLe_or w z ?? ??.
    + assume H1: w < z. exact H1.
    + assume H1: z <= w. prove False.
      apply SNoLt_irref 1.
      prove 1 < 1.
      apply SNoLt_tra 1 (x * z) 1 SNo_1 (SNo_mul_SNo x z ?? ??) SNo_1 Hz3.
      prove x * z < 1.
      apply SNoLeLt_tra (x * z) (x * w) 1 (SNo_mul_SNo x z ?? ??) (SNo_mul_SNo x w ?? ??) SNo_1.
      * prove x * z <= x * w.
        apply nonneg_mul_SNo_Le x z w ?? (SNoLtLe 0 x ??) ?? ??.
	exact H1.
      * prove x * w < 1. exact Hw3.
}
set y := SNoCut L R.
apply SNoCutP_SNoCut_impred L R LLR.
assume HLR1: SNo y.
...
assume HLR5.
claim LyL: y :e SNoS_ (ordsucc omega).
{ apply SNoS_I (ordsucc omega) ordsucc_omega_ordinal y (SNoLev y).
  - prove SNoLev y :e ordsucc omega.
    apply ordinal_In_Or_Subq (SNoLev y) (ordsucc omega)
                         (SNoLev_ordinal y HLR1) ordsucc_omega_ordinal.
    + assume H1: SNoLev y :e ordsucc omega. exact H1.
    + assume H1: ordsucc omega c= SNoLev y. prove False.
      claim L1: omega :e SNoLev y.
      { apply H1. apply ordsuccI2. }
      claim L2: TransSet ((\/_ x :e L, ordsucc (SNoLev x)) :\/: (\/_ y :e R, ordsucc (SNoLev y))).
      { apply ordinal_TransSet.
        apply ordinal_binunion.
        - apply ordinal_famunion.
	  let q. assume Hq: q :e L.
	  prove ordinal (ordsucc (SNoLev q)).
	  apply ordinal_ordsucc.
	  apply SNoLev_ordinal.
	  prove SNo q.
	  apply LLE q Hq. assume H _ _ . exact H.
	- apply ordinal_famunion.
	  let q. assume Hq: q :e R.
	  prove ordinal (ordsucc (SNoLev q)).
	  apply ordinal_ordsucc.
	  apply SNoLev_ordinal.
	  prove SNo q.
	  apply LRE q Hq. assume H _ _ . exact H.
      }
      claim L3: omega :e (\/_ x :e L, ordsucc (SNoLev x)) :\/: (\/_ y :e R, ordsucc (SNoLev y)).
      { exact TransSet_In_ordsucc_Subq (SNoLev y)
	           ((\/_ x :e L, ordsucc (SNoLev x)) :\/: (\/_ y :e R, ordsucc (SNoLev y)))
	           L2 HLR2 omega L1.
      }
      apply binunionE (\/_ x :e L, ordsucc (SNoLev x)) (\/_ y :e R, ordsucc (SNoLev y)) omega L3.
      * assume H2: omega :e \/_ x :e L, ordsucc (SNoLev x).
        apply famunionE_impred L (fun x => ordsucc (SNoLev x)) omega H2.
        let q. assume Hq: q :e L.
        assume H3: omega :e ordsucc (SNoLev q).
        apply In_no2cycle omega (ordsucc (SNoLev q)) H3.
        prove ordsucc (SNoLev q) :e omega.
        apply omega_ordsucc.
        prove SNoLev q :e omega.
        apply LLE q Hq. assume _ H _ . exact H.
      * assume H2: omega :e \/_ y :e R, ordsucc (SNoLev y).
        apply famunionE_impred R (fun x => ordsucc (SNoLev x)) omega H2.
        let q. assume Hq: q :e R.
        assume H3: omega :e ordsucc (SNoLev q).
        apply In_no2cycle omega (ordsucc (SNoLev q)) H3.
        prove ordsucc (SNoLev q) :e omega.
        apply omega_ordsucc.
        prove SNoLev q :e omega.
        apply LRE q Hq. assume _ H _ . exact H.
  - prove SNo_ (SNoLev y) y. apply SNoLev_. exact HLR1.
}
claim Ly: y :e real.
{ apply real_I.
  + prove y :e SNoS_ (ordsucc omega). exact LyL.
  + prove y <> omega.
    assume H1: y = omega.
    (** 0 < x and x real implies there is some n such that eps_ k such that eps_ k < x
        and then 2 ^ k is in R and so y < 2 ^ k < omega **)
    claim L4: 0 = x.
    { apply Hx6.
      - prove 0 :e SNoS_ omega. exact omega_SNoS_omega 0 (nat_p_omega 0 nat_0).
      - let k. assume Hk: k :e omega.
        prove abs_SNo (0 + - x) < eps_ k.
	rewrite add_SNo_0L (- x) (SNo_minus_SNo x ??).
	prove abs_SNo (- x) < eps_ k.
	rewrite abs_SNo_minus x ??.
	prove abs_SNo x < eps_ k.
	rewrite pos_abs_SNo x Hxpos.
	prove x < eps_ k.
	apply SNoLtLe_or x (eps_ k) ?? (SNo_eps_ k Hk).
	- assume H2: x < eps_ k. exact H2.
	- assume H2: eps_ k <= x. prove False.
	  apply SNoLt_irref omega.
	  prove omega < omega.
	  rewrite <- H1 at 1.
	  prove y < omega.
	  claim L2k: nat_p (2 ^ k).
	  ...
	  claim L2k': SNo (2 ^ k).
	  { exact nat_p_SNo (2 ^ k) L2k. }
	  claim L2k1: 2 ^ k + 1 :e omega.
	  { rewrite add_SNo_1_ordsucc (2 ^ k) (nat_p_omega (2 ^ k) L2k).
	    apply omega_ordsucc.
	    prove 2 ^ k :e omega. exact nat_p_omega (2 ^ k) L2k.
	  }
	  apply SNoLt_tra y (2 ^ k + 1) omega HLR1 (SNo_add_SNo (2 ^ k) 1 L2k' SNo_1) SNo_omega.
	  + prove y < 2 ^ k + 1.
	    apply HLR4.
	    prove 2 ^ k + 1 :e R.
	    prove 2 ^ k + 1 :e {q :e SNoS_ omega|1 < x * q}.
	    apply SepI.
	    * prove 2 ^ k + 1 :e SNoS_ omega. exact omega_SNoS_omega (2 ^ k + 1) L2k1.
	    * { prove 1 < x * (2 ^ k + 1).
	        rewrite mul_SNo_distrL x (2 ^ k) 1 ?? L2k' SNo_1.
		prove 1 < x * 2 ^ k + x * 1.
		rewrite mul_SNo_oneR x ??.
		prove 1 < x * 2 ^ k + x.
		apply SNoLtLe_tra 1 (1 + x) (x * 2 ^ k + x) SNo_1 (SNo_add_SNo 1 x SNo_1 ??) (SNo_add_SNo (x * 2 ^ k) x (SNo_mul_SNo x (2 ^ k) ?? L2k') ??).
		- prove 1 < 1 + x.
		  rewrite <- add_SNo_0R 1 SNo_1 at 1.
		  prove 1 + 0 < 1 + x.
		  exact add_SNo_Lt2 1 0 x SNo_1 SNo_0 ?? Hxpos.
		- prove 1 + x <= x * 2 ^ k + x.
		  apply add_SNo_Le1 1 x (x * 2 ^ k) SNo_1 ?? (SNo_mul_SNo x (2 ^ k) ?? L2k').
		  prove 1 <= x * 2 ^ k.
		  rewrite <- mul_SNo_eps_power_2 k (omega_nat_p k Hk) at 1.
		  prove eps_ k * 2 ^ k <= x * 2 ^ k.
		  apply nonneg_mul_SNo_Le' (eps_ k) x (2 ^ k).
		  + exact SNo_eps_ k Hk.
		  + exact ??.
                  + exact L2k'.
		  + prove 0 <= 2 ^ k. apply SNoLtLe.
		    exact exp_SNo_nat_pos 2 SNo_2 SNoLt_0_2 k (omega_nat_p k Hk).
		  + prove eps_ k <= x. exact H2.
	      }
	  + prove 2 ^ k + 1 < omega.
	    apply ordinal_In_SNoLt omega omega_ordinal.
	    prove 2 ^ k + 1 :e omega. exact L2k1.
    }
    apply SNoLt_irref 0.
    prove 0 < 0. rewrite L4 at 2. exact Hxpos.
  + prove y <> - omega.
    assume H1: y = - omega.
    apply SNoLt_irref 0.
    prove 0 < 0.
    apply SNoLt_tra 0 y 0 SNo_0 HLR1 SNo_0.
    * { prove 0 < y. apply HLR3. prove 0 :e L.
        prove 0 :e {q :e SNoS_ omega|x * q < 1}.
	apply SepI.
	- prove 0 :e SNoS_ omega. exact omega_SNoS_omega 0 (nat_p_omega 0 nat_0).
	- prove x * 0 < 1. rewrite mul_SNo_zeroR x ??. exact SNoLt_0_1.
      }
    * rewrite H1. prove - omega < 0.
      apply minus_SNo_Lt_contra1 0 omega SNo_0 SNo_omega.
      prove - 0 < omega. rewrite minus_SNo_0.
      prove 0 < omega.
      exact ordinal_In_SNoLt omega omega_ordinal 0 (nat_p_omega 0 nat_0).
  + let q.
    assume Hq1: q :e SNoS_ omega.
    assume Hq2: forall k :e omega, abs_SNo (q + - y) < eps_ k.
    apply SNoS_E2 omega omega_ordinal q Hq1.
    assume Hq1a Hq1b Hq1c Hq1d.
    claim Lq: q :e real.
    { exact SNoS_omega_real q Hq1. }
    claim Lxq: x * q :e real.
    { exact real_mul_SNo x Hx q Lq. }
    apply real_E (x * q) Lxq.
    assume Hxq1 Hxq2 Hxq3 Hxq4 Hxq5 Hxq6 Hxq7.
    claim Lmxq: SNo (- x * q).
    { exact SNo_minus_SNo (x * q) ??. }
    claim Lxq1: x * q < 1 -> exists n :e omega, x * q + eps_ n <= 1.
    { assume H1: x * q < 1. apply dneg.
      assume HC: ~exists n :e omega, x * q + eps_ n <= 1.
      claim Lxq1a: 1 = x * q.
      { apply Hxq6 1 (omega_SNoS_omega 1 (nat_p_omega 1 nat_1)).
        let n. assume Hn: n :e omega.
        prove abs_SNo (1 + - x * q) < eps_ n.
        rewrite pos_abs_SNo (1 + - x * q) (SNoLt_minus_pos (x * q) 1 ?? SNo_1 H1).
        prove 1 + - x * q < eps_ n.
        apply SNoLtLe_or (1 + - x * q) (eps_ n) (SNo_add_SNo 1 (- x * q) SNo_1 ??) (SNo_eps_ n Hn).
        - assume H2. exact H2.
        - assume H2: eps_ n <= 1 + - x * q. apply HC. witness n. apply andI.
          + exact Hn.
          + prove x * q + eps_ n <= 1.
            rewrite add_SNo_com (x * q) (eps_ n) ?? (SNo_eps_ n Hn).
            prove eps_ n + x * q <= 1.
            apply add_SNo_minus_Le2 1 (x * q) (eps_ n) SNo_1 ?? (SNo_eps_ n Hn).
            prove eps_ n <= 1 + - x * q.
            exact H2.
      }
      apply SNoLt_irref 1.
      prove 1 < 1. rewrite Lxq1a at 1. exact H1.
    }
    claim L1xq: 1 < x * q -> exists n :e omega, 1 <= x * q + - eps_ n.
    { assume H1: 1 < x * q. apply dneg.
      assume HC: ~exists n :e omega, 1 <= x * q + - eps_ n.
      claim L1xqa: 1 = x * q.
      { apply Hxq6 1 (omega_SNoS_omega 1 (nat_p_omega 1 nat_1)).
        let n. assume Hn: n :e omega.
        prove abs_SNo (1 + - x * q) < eps_ n.
        rewrite abs_SNo_dist_swap 1 (x * q) SNo_1 ??.
        prove abs_SNo (x * q + - 1) < eps_ n.
        rewrite pos_abs_SNo (x * q + - 1) (SNoLt_minus_pos 1 (x * q) SNo_1 ?? H1).
        prove x * q + - 1 < eps_ n.
        apply SNoLtLe_or (x * q + - 1) (eps_ n) (SNo_add_SNo (x * q) (- 1) ?? (SNo_minus_SNo 1 SNo_1)) (SNo_eps_ n Hn).
        - assume H2. exact H2.
        - assume H2: eps_ n <= x * q + - 1. apply HC. witness n. apply andI.
          + exact Hn.
          + prove 1 <= x * q + - eps_ n.
            apply add_SNo_minus_Le2b (x * q) (eps_ n) 1 ?? (SNo_eps_ n Hn) SNo_1.
            prove 1 + eps_ n <= x * q.
            rewrite add_SNo_com 1 (eps_ n) SNo_1 (SNo_eps_ n Hn).
            prove eps_ n + 1 <= x * q.
            apply add_SNo_minus_Le2 (x * q) 1 (eps_ n) ?? SNo_1 (SNo_eps_ n Hn).
            exact H2.
      }
      apply SNoLt_irref 1.
      prove 1 < 1. rewrite L1xqa at 2. exact H1.
    }
    prove q = y.
    apply SNoLt_trichotomy_or_impred q y Hq1c HLR1.
    * { assume H1: q < y. prove False.
        apply SNoLt_trichotomy_or_impred (x * q) 1 Hxq1 SNo_1.
        - assume H2: x * q < 1.
          apply Lxq1 H2.
          let n. assume H. apply H.
          assume Hn: n :e omega.
          assume H3: x * q + eps_ n <= 1.
          claim Lqeny: q + eps_ n < y.
          { apply HLR3.
            prove q + eps_ n :e L.
            apply SepI.
            - prove q + eps_ n :e SNoS_ omega. exact add_SNo_SNoS_omega q Hq1 (eps_ n) (SNo_eps_SNoS_omega n Hn).
            - prove x * (q + eps_ n) < 1.
              rewrite mul_SNo_distrL x q (eps_ n) ?? ?? (SNo_eps_ n Hn).
              prove x * q + x * eps_ n < 1.
              apply SNoLtLe_tra (x * q + x * eps_ n) (x * q + eps_ n) 1
                                (SNo_add_SNo (x * q) (x * eps_ n) ?? (SNo_mul_SNo x (eps_ n) ?? (SNo_eps_ n Hn)))
                                (SNo_add_SNo (x * q) (eps_ n) ?? (SNo_eps_ n Hn))
                                SNo_1.
              + prove x * q + x * eps_ n < x * q + eps_ n.
                apply add_SNo_Lt2 (x * q) (x * eps_ n) (eps_ n) ??
                                  (SNo_mul_SNo x (eps_ n) ?? (SNo_eps_ n Hn))
                                  (SNo_eps_ n Hn).
                prove x * eps_ n < eps_ n.
                exact mul_SNo_Lt1_pos_Lt x (eps_ n) ?? (SNo_eps_ n Hn) Hxsmall (SNo_eps_pos n Hn).
              + exact H3.
          }
          apply SNoLt_irref (q + eps_ n).
          apply SNoLt_tra (q + eps_ n) y (q + eps_ n)
                          (SNo_add_SNo q (eps_ n) ?? (SNo_eps_ n Hn))
                          HLR1
                          (SNo_add_SNo q (eps_ n) ?? (SNo_eps_ n Hn))
                          Lqeny.
          prove y < q + eps_ n.
          rewrite add_SNo_com q (eps_ n) ?? (SNo_eps_ n Hn).
          prove y < eps_ n + q.
          apply add_SNo_minus_Lt1 y q (eps_ n) HLR1 ?? (SNo_eps_ n Hn).
          prove y + - q < eps_ n.
          rewrite <- pos_abs_SNo (y + - q) (SNoLt_minus_pos q y ?? HLR1 H1).
          prove abs_SNo (y + - q) < eps_ n.
          rewrite abs_SNo_dist_swap y q HLR1 ??.
          prove abs_SNo (q + - y) < eps_ n.
          exact Hq2 n Hn.
        - assume H2: x * q = 1. prove False. apply HC.
          prove exists y :e real, x * y = 1.
          witness q. apply andI.
          + exact Lq.
          + exact H2.
        - assume H2: 1 < x * q.
          apply SNoLt_irref q.
          apply SNoLt_tra q y q ?? HLR1 ?? H1.
          prove y < q.
          apply HLR4.
          prove q :e R.
          apply SepI.
    ...
    * assume H1: q = y. exact H1.
    * { assume H1: y < q. prove False.
        apply SNoLt_trichotomy_or_impred (x * q) 1 Hxq1 SNo_1.
        - assume H2: x * q < 1.
          apply SNoLt_irref q.
          apply SNoLt_tra q y q ?? HLR1 ??.
          + prove q < y.
            apply HLR3.
            prove q :e L.
            apply SepI.
        ...
        - assume H2: 1 < x * q.
          ...
          let n. assume H. apply H.
          assume Hn: n :e omega.
          assume H3: 1 <= x * q + - eps_ n.
          claim Lyqen: y < q + - eps_ n.
          { apply HLR4.
            prove q + - eps_ n :e R.
            apply SepI.
            - prove q + - eps_ n :e SNoS_ omega.
              apply add_SNo_SNoS_omega q Hq1 (- eps_ n).
              prove - eps_ n :e SNoS_ omega.
              apply minus_SNo_SNoS_omega.
              exact SNo_eps_SNoS_omega n Hn.
            - prove 1 < x * (q + - eps_ n).
              rewrite mul_SNo_distrL x q (- eps_ n) ?? ?? (SNo_minus_SNo (eps_ n) (SNo_eps_ n Hn)).
              prove 1 < x * q + x * (- eps_ n).
              apply SNoLeLt_tra 1 (x * q + - eps_ n) (x * q + x * (- eps_ n)) SNo_1
                                (SNo_add_SNo (x * q) (- (eps_ n)) ?? (SNo_minus_SNo (eps_ n) (SNo_eps_ n Hn)))
                                (SNo_add_SNo (x * q) (x * (- eps_ n)) ?? (SNo_mul_SNo x (- eps_ n) ?? (SNo_minus_SNo (eps_ n) (SNo_eps_ n Hn)))) H3.
              prove x * q + - eps_ n < x * q + x * (- eps_ n).
              apply add_SNo_Lt2 (x * q) (- eps_ n) (x * (- eps_ n)) ??
                                (SNo_minus_SNo (eps_ n) (SNo_eps_ n Hn))
                                (SNo_mul_SNo x (- eps_ n) ?? (SNo_minus_SNo (eps_ n) (SNo_eps_ n Hn))).
                prove - eps_ n < x * (- eps_ n).
                rewrite mul_SNo_minus_distrR x (eps_ n) ?? (SNo_eps_ n Hn).
                prove - eps_ n < - x * eps_ n.
                apply minus_SNo_Lt_contra (x * eps_ n) (eps_ n) (SNo_mul_SNo x (eps_ n) ?? (SNo_eps_ n Hn)) (SNo_eps_ n Hn).
                prove x * eps_ n < eps_ n.
                exact mul_SNo_Lt1_pos_Lt x (eps_ n) ?? (SNo_eps_ n Hn) Hxsmall (SNo_eps_pos n Hn).
          }
          apply SNoLt_irref y.
          apply SNoLt_tra y (q + - eps_ n) y
                          HLR1
                          (SNo_add_SNo q (- eps_ n) ?? (SNo_minus_SNo (eps_ n) (SNo_eps_ n Hn)))
                          HLR1
                          Lyqen.
          prove q + - eps_ n < y.
          apply add_SNo_minus_Lt1b q (eps_ n) y ?? (SNo_eps_ n Hn) HLR1.
          prove q < y + eps_ n.
          rewrite add_SNo_com y (eps_ n) HLR1 (SNo_eps_ n Hn).
          prove q < eps_ n + y.
          apply add_SNo_minus_Lt1 q y (eps_ n) ?? HLR1 (SNo_eps_ n Hn).
          prove q + - y < eps_ n.
          rewrite <- pos_abs_SNo (q + - y) (SNoLt_minus_pos y q HLR1 ?? H1).
          prove abs_SNo (q + - y) < eps_ n.
          exact Hq2 n Hn.
      }
}
apply real_E y Ly.
assume Hy1 Hy2 Hy3 Hy4 Hy5 Hy6 Hy7.
claim Lxy: x * y :e real.
{ exact real_mul_SNo x Hx y Ly. }
claim Lxy': SNo (x * y).
{ exact real_SNo (x * y) Lxy. }
apply real_E (x * y) Lxy.
assume Hxy1: SNo (x * y).
assume Hxy2.
assume Hxy3.
assume Hxy4.
assume Hxy5.
assume Hxy6: forall q :e SNoS_ omega, (forall k :e omega, abs_SNo (q + - (x * y)) < eps_ k) -> q = x * y.
assume Hxy7: forall k :e omega, exists q :e SNoS_ omega, q < x * y /\ x * y < q + eps_ k.
apply HC.
witness y. apply andI.
- prove y :e real. exact Ly.
- prove x * y = 1.
  symmetry.
  apply Hxy6.
  + prove 1 :e SNoS_ omega. apply omega_SNoS_omega 1 (nat_p_omega 1 nat_1).
  + let k. assume Hk: k :e omega.
    prove abs_SNo (1 + - (x * y)) < eps_ k.
    apply Hy7 (ordsucc k) (omega_ordsucc k Hk).
    let q. assume H. apply H.
    assume Hq1: q :e SNoS_ omega.
    assume H. apply H.
    assume Hq2: q < y.
    ...
    assume Hq1a Hq1b Hq1c Hq1d.
    claim Lxq: SNo (x * q).
    { exact SNo_mul_SNo x q ?? ??. }
    claim LeSk: SNo (eps_ (ordsucc k)).
    { exact SNo_eps_ (ordsucc k) (omega_ordsucc k Hk). }
    claim LxeSk: SNo (x * eps_ (ordsucc k)).
    { exact SNo_mul_SNo x (eps_ (ordsucc k)) ?? ??. }
    claim LqeSk: SNo (q + eps_ (ordsucc k)).
    { exact SNo_add_SNo q (eps_ (ordsucc k)) ?? ??. }
    claim L1mxq: SNo (1 + - (x * q)).
    { exact SNo_add_SNo 1 (- (x * q)) SNo_1 (SNo_minus_SNo (x * q) ??). }
    claim Lxqmxy: SNo (x * q + - (x * y)).
    { exact SNo_add_SNo (x * q) (- (x * y)) ?? (SNo_minus_SNo (x * y) ??). }
    claim L1: x * (q + eps_ (ordsucc k)) < x * q + eps_ (ordsucc k).
    { rewrite mul_SNo_distrL x q (eps_ (ordsucc k)) ?? ?? ??.
      prove x * q + x * eps_ (ordsucc k) < x * q + eps_ (ordsucc k).
      apply add_SNo_Lt2 (x * q) (x * eps_ (ordsucc k)) (eps_ (ordsucc k)) ?? ?? ??.
      prove x * eps_ (ordsucc k) < eps_ (ordsucc k).
      apply mul_SNo_Lt1_pos_Lt x (eps_ (ordsucc k)) ?? ?? ??.
      exact SNo_eps_pos (ordsucc k) (omega_ordsucc k Hk).
    }
    apply SNoLeLt_tra (abs_SNo (1 + - (x * y)))
                      (abs_SNo (1 + - (x * q)) + abs_SNo (x * q + - (x * y)))
                      (eps_ k).
    * exact SNo_abs_SNo (1 + - x * y) (SNo_add_SNo 1 (- (x * y)) SNo_1 (SNo_minus_SNo (x * y) ??)).
    * exact SNo_add_SNo (abs_SNo (1 + - (x * q))) (abs_SNo (x * q + - (x * y)))
                        (SNo_abs_SNo (1 + - (x * q)) ??)
                        (SNo_abs_SNo (x * q + - (x * y)) ??).
    * exact SNo_eps_ k Hk.
    * exact SNo_triangle2 1 (x * q) (x * y) SNo_1 (SNo_mul_SNo x q ?? ??) (SNo_mul_SNo x y ?? ??).
    * { prove abs_SNo (1 + - x * q) + abs_SNo (x * q + - x * y) < eps_ k.
        rewrite <- eps_ordsucc_half_add k (omega_nat_p k Hk).
        prove abs_SNo (1 + - x * q) + abs_SNo (x * q + - x * y) < eps_ (ordsucc k) + eps_ (ordsucc k).
        apply add_SNo_Lt3.
        - exact SNo_abs_SNo (1 + - (x * q)) ??.
        - exact SNo_abs_SNo (x * q + - (x * y)) ??.
        - exact LeSk.
        - exact LeSk.
        - prove abs_SNo (1 + - x * q) < eps_ (ordsucc k).
          apply abs_SNo_intvl_bd (x * q) 1 (eps_ (ordsucc k)) ?? SNo_1 ??.
          + prove x * q <= 1.
            apply SNoLtLe_or 1 (x * q) SNo_1 Lxq.
            * { assume H1: 1 < x * q. prove False.
                apply SNoLt_irref q.
                prove q < q.
                apply SNoLt_tra q y q ?? ?? ?? Hq2.
                prove y < q.
                apply HLR4.
                prove q :e R.
                apply SepI.
                - prove q :e SNoS_ omega. exact Hq1.
                - prove 1 < x * q. exact H1.
              }
            * assume H1: x * q <= 1. exact H1.
          + prove 1 < x * q + eps_ (ordsucc k).
            apply SNoLeLt_tra 1 (x * (q + eps_ (ordsucc k))) (x * q + eps_ (ordsucc k))
                          SNo_1 (SNo_mul_SNo x (q + eps_ (ordsucc k)) ?? ??)
                          (SNo_add_SNo (x * q) (eps_ (ordsucc k)) ?? ??).
            * { prove 1 <= x * (q + eps_ (ordsucc k)).
                apply SNoLtLe_or (x * (q + eps_ (ordsucc k))) 1 
                                 (SNo_mul_SNo x (q + eps_ (ordsucc k)) ?? ??)
                                 SNo_1.
                - assume H1: x * (q + eps_ (ordsucc k)) < 1. prove False.
                  apply SNoLt_irref y.
                  prove y < y.
                  apply SNoLt_tra y (q + eps_ (ordsucc k)) y ?? ?? ?? Hq3.
                  prove q + eps_ (ordsucc k) < y.
                  apply HLR3.
                  prove q + eps_ (ordsucc k) :e L.
                  apply SepI.
                  + apply add_SNo_SNoS_omega.
                    * exact Hq1.
                    * exact SNo_eps_SNoS_omega (ordsucc k) (omega_ordsucc k Hk).
                  + prove x * (q + eps_ (ordsucc k)) < 1.
                    exact H1.
                - assume H1. exact H1.
              }
            * prove x * (q + eps_ (ordsucc k)) < x * q + eps_ (ordsucc k).
              exact L1.
        - prove abs_SNo (x * q + - x * y) < eps_ (ordsucc k).
          rewrite abs_SNo_dist_swap (x * q) (x * y) Lxq Lxy'.
          apply abs_SNo_intvl_bd (x * q) (x * y) (eps_ (ordsucc k)) ?? ?? ??.
          + prove x * q <= x * y. apply SNoLtLe.
            prove x * q < x * y.
            apply pos_mul_SNo_Lt x q y ?? ?? ?? ??.
            prove q < y. exact Hq2.
          + prove x * y < x * q + eps_ (ordsucc k).
            apply SNoLt_tra (x * y) (x * (q + eps_ (ordsucc k))) (x * q + eps_ (ordsucc k)) ?? (SNo_mul_SNo x (q + eps_ (ordsucc k)) ?? ??) (SNo_add_SNo (x * q) (eps_ (ordsucc k)) ?? ??).
            * apply pos_mul_SNo_Lt x y (q + eps_ (ordsucc k)) ?? ?? ?? ??.
              exact Hq3.
            * prove x * (q + eps_ (ordsucc k)) < x * q + eps_ (ordsucc k).
              exact L1.
      }
...
Theorem pos_real_recip_ex: forall x :e real, 0 < x -> exists y :e real, x * y = 1.
let x. assume Hx Hxpos.
apply real_E x Hx.
assume Hx1 Hx2 Hx3 Hx4 Hx5 Hx6 Hx7.
apply SNoS_ordsucc_omega_bdd_eps_pos x ?? Hxpos ??.
let N. assume H. apply H.
assume HN: N :e omega.
assume H1: eps_ N * x < 1.
claim L1: eps_ N :e real.
{ exact SNoS_omega_real (eps_ N) (SNo_eps_SNoS_omega N HN). }
claim L2: 0 < eps_ N * x.
{ exact mul_SNo_pos_pos (eps_ N) x (SNo_eps_ N HN) ?? (SNo_eps_pos N HN) ??. }
apply pos_small_real_recip_ex (eps_ N * x) (real_mul_SNo (eps_ N) ?? x ??) L2 H1.
let y. assume H. apply H.
assume Hy: y :e real.
assume H2: (eps_ N * x) * y = 1.
claim LN: SNo (eps_ N).
{ exact SNo_eps_ N HN. }
witness (eps_ N * y). apply andI.
- prove eps_ N * y :e real.
  apply real_mul_SNo.
  + apply SNoS_omega_real. prove eps_ N :e SNoS_ omega.
    exact SNo_eps_SNoS_omega N HN.
  + exact Hy.
- prove x * (eps_ N * y) = 1.
  rewrite mul_SNo_assoc x (eps_ N) y ?? ?? (real_SNo y Hy).
  prove (x * eps_ N) * y = 1.
  rewrite mul_SNo_com x (eps_ N) ?? ??.
  exact H2.
Qed.

Theorem nonzero_real_recip_ex: forall x :e real, x <> 0 -> exists y :e real, x * y = 1.
let x. assume Hx. assume Hxnz: x <> 0.
claim Lx: SNo x.
{ exact real_SNo x Hx. }
apply SNoLt_trichotomy_or_impred x 0 Lx SNo_0.
- assume H1: x < 0.
  ...
  apply pos_real_recip_ex (- x) (real_minus_SNo x Hx) L1.
  let y. assume H2. apply H2.
  assume Hy: y :e real.
  assume H2: (- x) * y = 1.
  witness (- y). apply andI.
  + exact real_minus_SNo y Hy.
  + prove x * (- y) = 1.
    rewrite mul_SNo_minus_distrR x y Lx (real_SNo y Hy).
    prove - (x * y) = 1.
    rewrite <- mul_SNo_minus_distrL x y Lx (real_SNo y Hy).
    exact H2.
- assume H1: x = 0. prove False. exact Hxnz H1.
- assume H1: 0 < x. exact pos_real_recip_ex x Hx H1.
Qed.
